...
table_corres = (table_corres.iloc[:, [1, 3, 2, 10, 5, 11]]
    .rename(columns={
...
notes_ex = (notes_ex.iloc[:, [6, 9, 10, 11, 12, 13]]
        .rename(columns={
...
for code08 in mapping.keys():
    for code25 in mapping[code08]["naf25"].keys():
        ...
        notes = unicodedata.normalize("NFKC", row.notes.iloc[0])  if pd.notna(row.notes.iloc[0]) else None
        comprend = unicodedata.normalize("NFKC",row.Comprend.iloc[0]) if pd.notna(row.Comprend.iloc[0]) else None
        comprend_aussi = unicodedata.normalize("NFKC", row["Comprend.aussi"].iloc[0]) if pd.notna(row["Comprend.aussi"].iloc[0]) else None
        comprend_all = "\n".join(filter(None, [comprend, comprend_aussi])) if any([comprend, comprend_aussi]) else None
        comprend_pas = unicodedata.normalize("NFKC",row["Ne.comprend.pas"].iloc[0]) if pd.notna(row["Ne.comprend.pas"].iloc[0])  else None
        notes_expli = {"notes": notes,
